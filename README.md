...
| 화면 | 하는 일 |
|---|---|
| 묻기 | **대화로 이어 묻기** → 답변(마크다운) → **"이 내용을 위키에 저장할까요?"** → 정리 미리보기 → 새 노트로 저장 또는 **기존 노트에 이어 쓰기**. 저장한 노트를 근거로 답하는 **내 노트 참고** 토글 |
| 노트 | 저장한 노트 목록, 제목·요약·태그로 찾기 |
| 노트 상세 | 요약·정리·원문 답변·연결(클릭 이동), **고치기·삭제**, 노션용 마크다운 내보내기, 저장된 원본 보기 |
...
npm run smoke          # 35항목: 저장·규격·연결·목록·그래프·대화·검색·이어쓰기·수정·삭제·내보내기
...
- 답변 스트리밍(한 글자씩 나오게)
- 임베딩 기반 검색·연결 추천 (지금은 단어 겹침 점수)
- 노션 API 업로드 실제 계정으로 확인 (`NOTION_TOKEN`, `NOTION_DB_ID`)
- 노트 이름 바꾸기(파일 이름까지) · 휴지통 비우기 화면
